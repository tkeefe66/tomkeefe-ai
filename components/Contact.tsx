...
export default function Contact() {
  return (
    <section id="contact" className="border-b border-border bg-surface/40 px-5 py-28 sm:px-8 sm:py-40">
      <h2 className="mb-10 font-display text-[clamp(2.25rem,7vw,5rem)] font-black uppercase leading-none tracking-[-0.02em] text-ink sm:mb-14">
        Contact
      </h2>
        ...
        {site.contact.map((link) =>
          link.comingSoon ? (
            ...
            <a
              key={link.label}
              href={link.href}
              className="group flex items-center justify-between py-5 font-display text-xl font-bold uppercase tracking-tight text-ink transition-colors hover:text-accent sm:py-6 sm:text-3xl"
            >
              <span>{link.label}</span>
              <span
